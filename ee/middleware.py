...
class AdminOAuth2Middleware:
    ...
    def _is_oauth2_verified(self, request: HttpRequest) -> bool:
        ...
        if not secrets.compare_digest(cookie_hash, hashlib.sha256(session_secret.encode()).hexdigest()):
            logger.error("admin_oauth2_cookie_mismatch", user=request.user.email)
            return False
...
def admin_oauth2_callback(request: HttpRequest) -> HttpResponse:
    ...
    if error:
        logger.error(
            "admin_oauth2_error", error=error, user=request.user.email if request.user.is_authenticated else "anonymous"
        )
    ...
    if not state or state != saved_state:
        logger.error(
            "admin_oauth2_invalid_state", user=request.user.email if request.user.is_authenticated else "anonymous"
        )
        return redirect("/admin/")
